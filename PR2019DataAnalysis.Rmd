---
title: "Github_Puerto Rico 2019 Soil and Root Analyses by Depth"
output:
  html_document:
    df_print: paged
    toc: yes
...
ggplot(data = pairedpase.sum, aes(x = Depth, y = mean)) + geom_bar(stat = "identity") + facet_grid(. ~ Site) + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width = 0.2) + 
  scale_x_discrete(limits=rev(c("0-10cm", "10-20cm", "20-30cm"))) + 
  scale_y_continuous(limits = c(0,15), breaks = c(0, 5, 10, 15)) + coord_flip() + 
  theme_bw() +
  theme(panel.grid = element_blank()) +
